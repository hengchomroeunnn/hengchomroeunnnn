<template>
  <div class="flex flex-wrap justify-between border-b-4 pt-2 pb-2 p-4">
    ...
    <div
      class="space-x-[80px] font-bca text-gray-500 text-xl pt-6 max-sm:space-x-[8px] max-sm:text-lg max-[400px]:text-[16px]">
      <nuxt-link
        to="/"
  ...
  <div class="flex flex-wrap justify-between">
    <div
      class="text-[40px] font-bca text-gray-900 m-4 max-[400px]:text-[25px] max-[480px]:text-[24px]">
      Good <span class="text-[#7ED957]">{{ greeting }}</span>
    </div>
    <div>
      <button>
        <Icon
          name="ion:cloudy-night"
          class="text-gray-500 text-[40px] m-4 max-[400px]:text-[25px] max-[480px]:text-[28px]" />
      </button>
      <button>
        <Icon
...
export default {
  setup() {
    ...
    setGreeting(); // Initialize the greeting
